...
def load_pretrained_weights(network, fname, verbose=False):
    ...
    blacklist = ['classification_branches', 'classifiers']

    model_dict = mod.state_dict()
    # verify that all but the segmentation layers have the same shape
    for key, _ in model_dict.items():
        if key.startswith('classification_branches') or key.startswith('classifiers'):
            continue
    ...
    pretrained_dict = {k: v for k, v in pretrained_dict.items()
                       if k in model_dict.keys() and all([i not in k for i in skip_strings_in_pretrained]) and all([i not in k for i in blacklist])}
